...
export const POST: APIRoute = async ({ request }) => {
  try {
    ...
    if (payload.match_id == null || payload.home_fouls == null || payload.away_fouls == null) {
      return new Response(
        JSON.stringify({ error: 'Datos incompletos para actualizar faltas' } as ApiErrorResponse),
        {
          status: 400,
    ...
    return new Response(
      JSON.stringify({
        error: 'Error al actualizar faltas del partido',
  ...
  try {
    ...
    if (error) {
      if (error.code === 'PGRST116') {
        return new Response(JSON.stringify({ home_fouls: 0, away_fouls: 0 }), {
          status: 200,
          headers: { 'Content-Type': 'application/json' },
        });
      }
    ...
    return new Response(
      JSON.stringify({
        error: 'Error al cargar faltas del partido',
        message: error.message,
      } as ApiErrorResponse),
