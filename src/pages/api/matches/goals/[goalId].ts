...
export const DELETE: APIRoute = async ({ params, request }) => {
  ...
  console.log(`[BACKEND GOALS DELETE] Extracted goalId: '${goalIdFromParams}', Type: ${typeof goalIdFromParams}`);

  if (!goalIdFromParams || goalIdFromParams === "undefined" || goalIdFromParams.trim() === "") {
    console.error("[BACKEND GOALS DELETE] Invalid goalId (undefined, empty, or not provided):", goalIdFromParams);
    return new Response(JSON.stringify({ error: "Falta el ID del gol o es inválido" } as ApiErrorResponse), {
  ...
  if (isNaN(numericGoalId)) {
      ...
      return new Response(JSON.stringify({ error: `El ID del gol '${goalIdFromParams}' no es un número válido.` } as ApiErrorResponse), {
          status: 400, headers: { 'Content-Type': 'application/json' }
      });
  }
  ...
  try {
    ...
    console.log(`[BACKEND GOALS DELETE] Supabase delete successful. Rows affected (count): ${count}`);
    if (count === 0) {
        console.warn(`[BACKEND GOALS DELETE] No goal found with ID ${numericGoalId} to delete (count was 0).`);
    }
