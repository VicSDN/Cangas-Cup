...
export const GET: APIRoute = async ({ request }) => {
  ...
  try {
    ...
    if (error) {
      ...
      return new Response(JSON.stringify({ message: `Error de Supabase: ${error.message}` }), {
        status: 500,
      });
    }
    ...
    return new Response(JSON.stringify({ message: error.message || 'Error al obtener goles' }), {
      status: 500,
    });
  }
};
...
export const POST: APIRoute = async ({ request }) => {
  try {
    ...
    if (!match_id || !player_id || !team_id || !year) {
      return new Response(
        JSON.stringify({
          message: 'Faltan campos requeridos (match_id, player_id, team_id, year)',
        }),
        { status: 400 }
      );
    }
    ...
    if (error) {
      ...
      return new Response(
        JSON.stringify({ message: `Error de Supabase al insertar: ${error.message}` }),
        { status: 500 }
      );
    }
    ...
    return new Response(JSON.stringify({ message: error.message || 'Error al registrar el gol' }), {
      status: 500,
    });
  }
};
