...
export const POST: APIRoute = async ({ request }) => {
    ...
    if (!payload.match_id || !payload.year) {
      return new Response(JSON.stringify({ error: 'Match ID and year are required.' }), { status: 400 });
    }
    ...
    if (error) {
      ...
      return new Response(JSON.stringify({ error: error.message || 'Failed to update MVPs.' }), { status: 500 });
    }

    return new Response(JSON.stringify({ message: 'MVPs updated successfully.', data }), { status: 200 });
  } catch (e: any) {
