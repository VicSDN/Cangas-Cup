...
export const POST: APIRoute = async ({ request }) => {
  try {
    ...
    const { error } = await supabase.from('tournament_player').delete().eq('id', player_id);
    ...
    if (error) {
      ...
      if (error.code && error.code.startsWith('23')) {
        return new Response(
          JSON.stringify({
            error:
              'No se puede eliminar el jugador porque tiene datos asociados (tarjetas, goles, etc.). Elimina esos datos primero o configura eliminación en cascada.',
    ...
    return new Response(
      JSON.stringify({
        error: 'Error interno al eliminar el jugador',
