...
export const POST: APIRoute = async ({ request, cookies }) => {
  try {
    ...
    if (deleteMatchError) {
      ...
      return new Response(
        JSON.stringify({
          error: 'Error al eliminar el partido de la base de datos.',
          details: deleteMatchError.message,
        }),
        { status: 500 }
      );
    }
...
export const ALL: APIRoute = ({ request }) => {
  return new Response(
    JSON.stringify({ error: `Método ${request.method} no permitido en esta ruta.` }),
    {
      status: 405,
